...
[bau_supplies_01, "Land_SandbagBarricade_01_F", 10] call grad_fortifications_fnc_addFort;

// Kiste 7 - Verteidigungsanlagen in Zelenogorsk #
[bau_supplies_07, "Land_Razorwire_F", 5] call grad_fortifications_fnc_addFort;
[bau_supplies_07, "Land_BagFence_Short_F", 10] call grad_fortifications_fnc_addFort;
...
} forEach [bau_supplies_01, bau_supplies_07];
...
ASL_SUPPORTED_VEHICLES_OVERRIDE = ["UK3CB_BAF_Merlin_HC3_18_GPMG_DPMW"];  
 ...
 [] spawn {
    ...
    {
        private _houseArray = nearestTerrainObjects [_x, ["HOUSE"], 10]; 

        (_houseArray select 0) setDamage 1;
    } forEach [helper_01, helper_02, helper_03, helper_04, helper_05, helper_06, helper_07, helper_08, helper_09, helper_10, helper_11, helper_12, helper_13];

    sleep 10;
    //Türen abschließen
    {
        private _houseArray = nearestTerrainObjects [_x, ["house","building", "house_f"], 10];
        private _house = (_houseArray select 0);

        _nbDoors = getNumber (configfile >> "CfgVehicles" >> typeOf _house >> "numberOfDoors");
        for "_i" from 0 to _nbDoors - 2 do {
            _var = format ["bis_disabled_Door_%1", _i];
            (_house) setVariable [_var, 1, true]
        }
    } forEach [door_01, door_02, door_03, door_04];        
};